...
class Series:
    ...
    def simulate_lap(self, entrant: Entrant) -> tuple[int]:
        # Stint Ranges
        base_stint: int = entrant.driver.driver_score + entrant.tyre_performance + (entrant.car_ovr - 700)
        ...
        # print(f"{entrant.driver.driver_name}: {base_stint} / {min_stint} / {max_stint} / {stint_score}")
    ...
    def simulate_race_lap(self, entrant: Entrant, laps_remaining: int) -> list[int]:
        # Stint Ranges
        base_stint: int = entrant.driver.driver_score + entrant.tyre_performance + entrant.practice_boost + (entrant.car_ovr - 700)
        min_stint: int = Series.min_stint + base_stint
        max_stint: int = Series.max_stint + base_stint + entrant.setup_boost
        ...
        if crash_score == max_crash_chance:
            chance = random.randint(1, int(1.5 * Series.dnf_chance))
    ...
    def set_grid(self) -> None:
        ...
        for i in range(len(self.entrants)):
            ...
            entrant.spine_reliability = entrant.team.spine.reliability
            entrant.engine_reliability = entrant.team.engine.reliability
    ...
    def simulate_race(self, race_number: str) -> None:
        ...
        self.reliability_divisor = int(self.race_length * 1.6)
