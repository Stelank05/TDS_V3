...
class Series:
    ...
    def __init__(self, series_data: list[str], calendar: list[Event], entry_list_file: str) -> None:
        ...
        self.do_bumping: bool = series_data[6].lower() == "true"

        self.max_entries: int = int(series_data[7])

        self.log_base: float = float(series_data[8])
        self.subtractor: int = int(series_data[9])
    ...
    def simulate_event(self, event: Event, entrants: list[Entrant], event_folder: str) -> None:
        ...
        # Scrutineering
        self.scrutineering()
        if self.do_bumping and len(self.entrants) > self.max_entries:self.set_non_starters()
        ...
        # Drop Entries
        if self.do_bumping and len(self.entrants) > self.max_entries: self.drop_entrants()
        ...
        if not self.current_event.is_oval:
    ...
    def simulate_lap(self, entrant: Entrant) -> tuple[int]:
        # Stint Ranges
        base_stint: float = entrant.driver.driver_score + entrant.tyre_performance + entrant.car_ovr_log
        min_stint: float = Series.min_stint + base_stint
        max_stint: float = Series.max_stint + base_stint + entrant.setup_boost
        ...
        stint_score: float = round(random.uniform(min_stint, max_stint), 1)
        ...
        if incident < Series.min_incident + (2 * entrant.driver.crash_score):
            ...
            if dnf == Series.min_dnf: return 1.0, 0
            else:
                stint_score -= dnf
    ...
    def simulate_race_lap(self, entrant: Entrant, laps_remaining: int) -> list[int]:
        # Stint Ranges
        base_stint: float = entrant.driver.driver_score + entrant.tyre_performance + entrant.practice_boost + entrant.car_ovr_log
        min_stint: float = Series.min_stint + base_stint
        max_stint: float = Series.max_stint + base_stint + entrant.setup_boost
        ...
        stint: float = round(random.uniform(min_stint, max_stint), 1)
        tyre_wear: int = random.randint(min_tyre_wear, max_tyre_wear)
            ...
            for entrant in self.entrants:
                if entrant.state == RunningState.RUNNING:
                    ...
                    if entrant.state == RunningState.RUNNING:
                        ...
                        entrant.main_ovr = round(entrant.main_ovr + (stint[0] - stint[4]), 1)
