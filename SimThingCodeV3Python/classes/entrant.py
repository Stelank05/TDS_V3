...
class Entrant:
    def __init__(self, entrant_data: list[type]) -> None:
        ...
        self.backup_ovr: int
        self.car_ovr: int
        self.car_ovr_log: int

        self.practice_boost: int
        self.practice_score: int
    ...
    def set_ovr(self, venue: Track, do_bop: bool, log_base: float, subtractor: int, balance_table: BalanceTable = None) -> None:
        aero_perf: int = self.team.aero.total_ovr
        chassis_perf: int = self.team.spine.chassis
        sus_perf: int = self.team.spine.total_sus
        engine_perf: int = self.team.engine.total_ovr

        if do_bop:
            aero_perf -= (balance_table.front_aero + balance_table.rear_aero)
            chassis_perf -= balance_table.chassis
            engine_perf -= (balance_table.engine + balance_table.hybrid)
        ...
        # print(f"{self.car_no} {self.team.team_code}: {aero_perf} + {chassis_perf} + {sus_perf} + {engine_perf} + {self.team.tyres.performance} + {self.setup_boost} + {self.component_boost} = {self.car_ovr}")
        ...
        self.car_ovr_log = int(round(math.log(self.car_ovr, log_base), 0) - subtractor)
    ...
    def replace_driver(self, session: str, new_pos: int) -> None:
        ...
        self.backup_ovr = self.backup_ovr - self.driver.driver_score
    ...
    def create_file_line(self, disp_score: bool, session: str) -> str:
        ...
        if disp_score:
            if self.state == RunningState.RUNNING or self.state == RunningState.DID_NOT_QUALIFY: return_line = f"{return_line},{self.main_ovr}"
